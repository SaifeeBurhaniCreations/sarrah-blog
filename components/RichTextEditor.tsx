...
export const RichTextEditor: React.FC<RichTextEditorProps> = ({ value, onChange, placeholder }) => {
  ...
  const execCommand = (command: string, value: string | undefined = undefined) => {
    // Shared remove button style: ensure flex center, fixed size, no shrink
    const removeBtnStyle = "remove-component-btn absolute z-50 bg-white text-red-500 w-8 h-8 flex items-center justify-center rounded-full shadow-md opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-50 cursor-pointer shrink-0";
    const removeIcon = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`;
    ...
    if (command === 'formatBlock' && value === 'BLOCKQUOTE') {
        ...
        <div class="removable-component relative group my-8 pl-6 border-l-4 border-luxe-gold" contenteditable="false">
             <button class="${removeBtnStyle} -top-4 -right-4" title="Remove Quote">${removeIcon}</button>
        ...
        document.execCommand('insertHTML', false, quoteHtml);
         ...
         const dividerHtml = `
         <div class="removable-component relative group my-8 w-full" contenteditable="false">
             <button class="${removeBtnStyle} -top-4 right-1/2 translate-x-1/2" title="Remove Divider">${removeIcon}</button>
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        // Revised Remove Button with proper styling
        const removeButton = `
            <button class="remove-component-btn absolute top-2 right-2 z-50 bg-white text-red-500 w-8 h-8 flex items-center justify-center rounded-full shadow-md opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-50 cursor-pointer shrink-0" title="Remove Component">
        ...
        if (insertType === 'image') {
            ...
            <div class="removable-component my-8 group relative w-full block clear-both" contenteditable="false">
                ${removeButton}
                <img src="${uploads[0]}" class="w-full h-auto rounded-lg shadow-lg block max-w-full" alt="Editorial Image" />
            ...
            htmlToInsert = `
            <div class="removable-component float-right ml-4 mb-4 p-3 bg-white shadow-[0_10px_30px_-5px_rgba(0,0,0,0.15)] rotate-2 border border-gray-100 max-w-[180px] md:max-w-xs rounded-sm transition-transform hover:rotate-0 z-10 relative group" contenteditable="false">
                ${removeButton}
            ...
            <div class="removable-component my-10 w-full block clear-both relative group" contenteditable="false">
                ${removeButton}
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">${imagesHtml}</div>
            ...
            <p><br/></p>`;
             ...
             const imagesHtml = uploads.map(url => 
                `<div class="min-w-[250px] md:min-w-[300px] h-[300px] md:h-[400px] snap-center rounded-lg overflow-hidden flex-shrink-0 relative">
                    <img src="${url}" class="w-full h-full object-cover" />
                 </div>`
             ).join('');
             htmlToInsert = `
             <p><br/></p>
             <div class="removable-component my-12 relative group w-full block clear-both overflow-hidden" contenteditable="false">
                 ${removeButton}
  ...
  const insertProductCarousel = () => {
      ...
      const itemsHtml = productItems.map(item => `
        <div class="min-w-[260px] md:min-w-[280px] max-w-[300px] snap-center group/card perspective-1000 pl-4 h-full flex flex-col">
      ...
      const carouselHtml = `
      <p><br/></p>
      <div class="removable-component my-16 py-10 bg-gradient-to-b from-white via-luxe-cream/50 to-white border-y border-luxe-gold/10 relative w-full block clear-both group overflow-hidden" contenteditable="false">
          <button class="remove-component-btn absolute top-2 right-2 z-50 bg-white text-red-500 w-8 h-8 flex items-center justify-center rounded-full shadow-md opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-50 cursor-pointer shrink-0" title="Remove Product Carousel">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
          </button>
  ...
  const ToolbarButton = ({ icon: Icon, command, arg, label, isActive, onClick }: any) => (
    ...
    >
      <Icon size={18} />
      <span className="hidden md:block absolute -bottom-8 left-1/2 -translate-x-1/2 bg-gray-800 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none font-sans">
          {label}
      </span>
    </button>
  );
  ...
  return (
    <div className="border border-gray-200 rounded-lg bg-white focus-within:border-luxe-gold focus-within:ring-1 focus-within:ring-luxe-gold transition-all shadow-sm relative z-0">
      ...
      {/* Toolbar */}
      <div className="flex flex-wrap items-center gap-2 md:gap-1 p-2 border-b border-gray-100 bg-gray-50/80 backdrop-blur-sm sticky top-0 z-40 rounded-t-lg">
        ...
        <div className="hidden md:block w-[1px] h-5 bg-gray-300 mx-1"></div>
      ...
      <div
        ...
        className="min-h-[500px] p-4 md:p-8 focus:outline-none prose prose-slate max-w-none 
          prose-headings:font-serif prose-headings:font-normal prose-headings:text-luxe-black
          prose-h2:text-2xl md:prose-h2:text-3xl prose-h2:mt-10 prose-h2:mb-6
          prose-h3:text-lg md:prose-h3:text-xl prose-h3:mt-8 prose-h3:mb-4 prose-h3:uppercase prose-h3:tracking-wide
      ...
      {!value && (
         <div className="absolute top-[100px] md:top-[110px] left-4 md:left-8 text-gray-300 pointer-events-none font-serif italic text-lg select-none">
            {placeholder}
         </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4">
           <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
               <div className="p-4 md:p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                  <h3 className="font-serif text-xl md:text-2xl">Insert Product Carousel</h3>
               ...
               <div className="p-4 md:p-6 overflow-y-auto flex-1">
                   ...
                   <div className="bg-luxe-cream/50 p-4 rounded-lg border border-luxe-gold/10 mb-6">
                       ...
                       <div className="flex flex-col md:flex-row gap-4 items-end">
                           <div className="w-full md:w-20 h-20 bg-white border border-dashed border-gray-300 rounded-lg flex items-center justify-center cursor-pointer hover:border-luxe-gold relative overflow-hidden shrink-0">
                           ...
                           <Button variant="primary" onClick={handleAddProduct} disabled={isUploadingProduct} className="py-2 px-4 text-xs w-full md:w-auto">
               ...
               <div className="p-4 md:p-6 border-t border-gray-100 bg-gray-50 flex flex-col md:flex-row justify-end gap-4">
                   <Button variant="outline" onClick={() => setIsProductModalOpen(false)} className="w-full md:w-auto justify-center">Cancel</Button>
                   <Button variant="primary" onClick={insertProductCarousel} disabled={productItems.length === 0} className="w-full md:w-auto justify-center">
